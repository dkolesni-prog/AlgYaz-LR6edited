...
//ЛР6:
//+Так просто с бинарным вводом и выводом, увы,
// не получится, т.к. sizeof вернёт не то, что вы от него
// ожидаете. Подробнее см. в доп. материалах 3 https://drive.google.com/file/d/10LTIDxN0OZRjMCcafDPB..
//+К тому же, вы перепутали функцию чтения с функцией записи(

size_t size_of(const Citizen& obj)
{
    return obj.initials.length() + sizeof(obj.place.street) + sizeof(obj.place.building) + sizeof(obj.place.apartment) +
    sizeof(obj.age)+ obj.gender.length();
}

void bin_write (std::ofstream& to, std::vector<Citizen>& vec){
    for (int i = 0; i < vec.size(); ++i) {
        to.write(reinterpret_cast<char*>(vec[i].initials.data()), STR_LEN);
        to.write(reinterpret_cast<char*>(vec[i].place.street.data()), STR_LEN);
        to.write(reinterpret_cast<char*>(&vec[i].place.building), sizeof(vec[i].place.building));
        to.write(reinterpret_cast<char*>(&vec[i].place.apartment), sizeof(vec[i].place.apartment));
        to.write(reinterpret_cast<char*>(vec[i].gender.data()), STR_LEN);
        to.write(reinterpret_cast<char*>(&vec[i].age), sizeof(vec[i].age));
    }
}
...
void bin_read (std::istream& to, std::vector<Citizen>& vec){
    Citizen tmp;
    for (int i = 0; i < vec.size(); ++i) {
        to.read(reinterpret_cast<char*>(vec[i].initials.data()), STR_LEN);
        to.read(reinterpret_cast<char*>(vec[i].place.street.data()), STR_LEN);
        to.read(reinterpret_cast<char*>(&vec[i].place.building), sizeof(vec[i].place.building));
        to.read(reinterpret_cast<char*>(&vec[i].place.apartment), sizeof(vec[i].place.apartment));
        to.read(reinterpret_cast<char*>(vec[i].gender.data()), STR_LEN);
        to.read(reinterpret_cast<char*>(&vec[i].age), sizeof(vec[i].age));
    }
    vec.push_back((tmp));
}
int main() {
    ...
    std::ofstream bin_in ("in", std::ios::binary);
    if(bin_in.is_open()) {
        bin_write(bin_in, my_zhek);
        bin_in.close();
    }

    std::ifstream bin_out ("out", std::ios::binary);
    if (bin_out.is_open()) {
